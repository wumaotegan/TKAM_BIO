...
def load_mixed_finetuning_dataset(
    ...
    logger.info(">>> Step 1: Loading invivo target data (keeping oversampling)...")
    ...
    for config in target_configs:
        ...
        if balance_target:
            ...
            if n_pos > 0 and n_neg > 0:
                ...
                logger.info(f"  - Task [{task}]: {len(df)} samples after balancing (Pos={target_n}, Neg={target_n})")
    ...
    if replay_ratio == 0.0:
        logger.info(">>> Step 3: Replay ratio is 0; skipping replay sampling.")
        task_names_list = sorted(list(task_names))
        logger.info(f"Fine-tuning dataset ready. Total samples: {len(all_smiles)} (Tasks: {len(task_names_list)})")
        return all_smiles, all_labels_list, task_names_list
    
    logger.info(f">>> Step 2: Calculating replay budget (Target: {target_count}, Ratio: {replay_ratio}) -> Need: {n_replay_needed}")
    logger.info(">>> Step 3: Starting balanced sampling of replay tasks...")
    
    valid_replay_configs = [c for c in replay_configs if os.path.exists(c['path'])]
    if not valid_replay_configs:
        logger.warning("No valid replay configuration paths were found.")
    ...
    # -----------------------------------------------------------
    task_names_list = sorted(list(task_names))
    logger.info(f"Dataset ready. Final size: {len(all_smiles)} (Target: {target_count}, Replay: {len(selected_replay)})")
...
def get_test_loader(df_invivo_pfas_test,df_invivo_cf_test,replay_configs,all_task_names,batch_size,tokenizer):
    ...
    smiles_list = []
    labels_list = []
    print(f"Building test DataLoader from {len(current_test_configs)} source files...")
...
def save_huggingface_model(model, tokenizer, output_dir, pruned_tasks=None):
    """
    Save the model and record pruned tasks in config.json.
    """
...
def load_huggingface_model(model_dir, device="cpu"):
    ...
    if len(pruned_tasks) > 0:
        print(f"Detected {len(pruned_tasks)} pruned tasks in this model: {pruned_tasks}")
...
def get_model(model_name, new_task_names, device):
    ...
    print(f"Extending model with new tasks: {new_task_names}")
    ...
    print(f"Model extension complete. Total tasks: {len(model.task_names)}, pruned tasks: {len(pruned_tasks)}")
    ...
    print("Model configuration complete: new tasks use raw loss and old tasks use frozen Kendall loss.")
    return model, tokenizer
...
def run(model_name,seed,replay_ratio,train_batch_size,pfas_weight,cf_weight,bio_weight,epochs,encoder_lr,head_lr,device,accumulation_steps=4):
    new_task_names = ["Invivo_PFAS", "Invivo_CF"] 
    
    model,tokenizer = get_model(model_name=model_name, new_task_names=new_task_names, device=device)
    pruned_tasks=model.pruned_tasks
    ...
    train_dataloader,replay_configs = get_train_dataloader(invivo_pfas_train,invivo_cf_train,replay_ratio=replay_ratio,train_batch_size=train_batch_size,tokenizer=tokenizer,seed=seed,task_names=model.task_names,balance_target=False)
    ...
    final_cf_auc=0.0
    final_pfas_mcc=0.0
    final_bio_auc_mean=0.0

    print(f">>> Starting fine-tuning (Epochs: {epochs}, Seed: {seed}, Total Steps: {total_steps})...")
    ...
    for epoch in range(epochs):
        ...
        bioassay_auc = []
        for task, auc in scores.items():
            if "aid:" in task.lower() and "mcc" not in task.lower():
                bioassay_auc.append(auc)  
        curr_lr_head = optimizer.param_groups[1]['lr']
        print(f"Epoch {epoch+1:02d} | Loss: {avg_loss:.4f} | LR(Head): {curr_lr_head:.2e} | PFAS(AUC): {pfas_auc:.4f},PFAS(MCC): {pfas_mcc:.4f} | CF(AUC): {cf_auc:.4f},CF(MCC): {cf_mcc:.4f} | Bio(avg, tasknums:{len(bioassay_auc)}): {np.nanmean(bioassay_auc):.4f}")
        
        avg_score = (pfas_auc+cf_auc)/2
        if  avg_score > best_avg_score:
            best_avg_score = avg_score
            best_pfas_auc = pfas_auc
            final_cf_auc=cf_auc
            final_pfas_mcc=pfas_mcc
            final_bio_auc_mean=np.nanmean(bioassay_auc)
            save_huggingface_model(model, tokenizer, f"./saved_finetune_invivo_model/best_finetuned_invivo_model_seed_{seed}_replay_{replay_ratio}_pfasweight_{pfas_weight}_cfweight_{cf_weight}_bioweight_{bio_weight}.pth", pruned_tasks)

    print(f"Training complete. Best validation AUC: {best_pfas_auc:.4f}")
    return model,best_pfas_auc,final_cf_auc,final_pfas_mcc,final_bio_auc_mean


seeds = [42, 123, 999, 2026, 888]
def summarize_experiment_results(results):
    data = pd.DataFrame(results)
    metrics = ["pfas_auc", "pfas_mcc", "bio_auc", "cf_auc", "cf_mcc"]

    best_epochs_idx = data.groupby(
        ["Seed", "Strategy", "Replay_Ratio"]
    )["pfas_auc"].idxmax()
    best_data = data.loc[best_epochs_idx].copy()

    summary_rows = []
    for (strategy, replay_ratio), group in best_data.groupby(
        ["Strategy", "Replay_Ratio"], sort=True
    ):
        row = {
            "Strategy": strategy,
            "Replay_Ratio": replay_ratio,
        ...
        for metric in metrics:
            row[metric] = (
                ...
                f"{group[metric].std():.4f}"
            )
        row["avg_stopping_epoch"] = f"{group['epoch'].mean():.1f}"
...
def run_single_experiment(
    ...
    return best_metrics, model, tokenizer
...
def main_experiment():
    ...
    for seed in seeds:
        ...
        print(f"\n>>> [S1] No Replay (Baseline)")
        res1, _, _ = run_single_experiment(
            model_name_or_path=base_model_path,
            seed=seed,
            replay_ratio=0.0,
            pfas_weight=5.0, cf_weight=1.0, bio_weight=0.0,
            **common_params
        )
        
        row_s1 = {
            "Seed": seed, "Strategy": "1_NoReplay", "Replay_Ratio": 0.0, 
            **res1 
        }
        all_results.append(row_s1)
        
        torch.cuda.empty_cache()

        # ------------------------------------------------------------------
        # Strategy 2: Data Replay (Varying Ratios)
        # ------------------------------------------------------------------
        for ratio in replay_ratios_to_test:
            print(f"\n>>> [S2] Data Replay (Ratio={ratio})")
            res2, _, _ = run_single_experiment(
                model_name_or_path=base_model_path,
                seed=seed,
                replay_ratio=ratio,
            ...
            row_s2 = {
                "Seed": seed, "Strategy": "2_DataReplay", "Replay_Ratio": ratio, 
                **res2
            }
            all_results.append(row_s2)
            
            torch.cuda.empty_cache()

        # ------------------------------------------------------------------
        # Strategy 3: Curriculum Standard (No Replay)
        # ------------------------------------------------------------------
        print(f"\n>>> [S3] Curriculum Standard (No Replay)")
        
        print("  ...Phase 1 (CF)...")
        _, model_s3, tokenizer_s3 = run_single_experiment(
            model_name_or_path=base_model_path,
            seed=seed,
            replay_ratio=0.0,
        ...
        print("  ...Phase 2 (PFAS)...")
        res3, _, _ = run_single_experiment(
            model_name_or_path=None,
            input_model=model_s3, input_tokenizer=tokenizer_s3,
            seed=seed,
            replay_ratio=0.0,
            pfas_weight=5.0, cf_weight=1.0, bio_weight=0.0,
            **common_params
        ...
        row_s3 = {
            "Seed": seed, "Strategy": "3_Curriculum", "Replay_Ratio": 0.0, 
            **res3
        }
        all_results.append(row_s3)
        
        del model_s3, tokenizer_s3
        torch.cuda.empty_cache()
        gc.collect()

        # ------------------------------------------------------------------
        # Strategy 4: Curriculum + Bioassay (Varying Ratios)
        # ------------------------------------------------------------------
        print(f"\n>>> [S4] Curriculum + Bioassay (Preparing Phase 1...)")

        print("  ...Phase 1 (CF Common Base)...")
        _, model_s4_base, tokenizer_s4_base = run_single_experiment(
            model_name_or_path=base_model_path,
            seed=seed,
            replay_ratio=0.0,
            pfas_weight=0.0, cf_weight=1.0, bio_weight=0.0,
        ...
        for ratio in replay_ratios_to_test:
            ...
            res4, _, _ = run_single_experiment(
                model_name_or_path=None,
    ...
    formatted_summary = summarize_experiment_results(all_results)
...
if __name__ == "__main__":
    ...
    formatted_summary = main_experiment()
